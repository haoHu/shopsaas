(function ($) {
	...
	Hualala.Common.serializeForm = function (form) {
		var $form = $(form);
		if ($form.length == 0) return null;
		var fields = _.groupBy($form.serializeArray(), 'name');
		var ret = _.mapObject(fields, function (v, k) {
			var r = _.pluck(v, 'value');
			return r.length == 0 ? '' : (r.length == 1  ? r[0] : r);
		});
		IX.Debug.info("Serialize form fields:");
		IX.Debug.info(ret);
		return ret;
	};
